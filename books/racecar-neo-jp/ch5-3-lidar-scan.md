...
- シミュレータでは 720個の小数（float）が入っている（実機は 1080個。9-3）
- 0番めは車の**真正面**。そこから**時計回り**に進む（720個なら 0.5° ずつ）
- 単位は **cm**
- **0.0 は「データなし」**。近すぎる・遠すぎる・光が返ってこない、などで測れなかった点

### 番号と角度を行き来する

番号 $i$ の点の角度と、角度 $\theta$ の点の番号は、次の式で行き来できます。`len(scan)` は点の数（シミュレータでは 720、実機では 1080）です。
